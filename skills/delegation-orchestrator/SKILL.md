...
默认会同时安装 `record-and-reflect-review`、`task-retrospective`、`delegation-orchestrator` 和 `browser-session-manager`，并更新全局 `AGENTS.md`，让新线程默认进入委派协作模式。如果只想单独安装本 skill，可执行：
